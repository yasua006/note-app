...
export const handle_note_edits = async (edit_note_btn) => {
    ...
    const matching_note_desc = main_elem.querySelector(desc_id).textContent;
    const edit_title_prompt = prompt(`Current title: ${matching_note_title}`);
    const edit_desc_prompt = prompt(`Current description: ${matching_note_desc}`);
    if (!edit_title_prompt)
        return;
    if (!edit_desc_prompt)
        return;
    try {
        const res = await fetch(`/patch-note?title=${edit_title_prompt}&description=${edit_desc_prompt}&id=${matching_note_id}`, { method: "PATCH" });
        if (!res.ok) {
...
export const handle_todo_edits = async (edit_todo_btn) => {
    const id = edit_todo_btn.id.replaceAll("edit-todo-btn-", "todo-id-");
    ...
    if (!edit_task_done_prompt)
        return;
    try {
        const res = await fetch(`/patch-todo?title=${edit_title_prompt}&description=${edit_desc_prompt}&task_done=${edit_task_done_prompt}&id=${matching_todo_id}`, { method: "PATCH" });
        if (!res.ok) {
